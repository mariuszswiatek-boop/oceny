...
export default function WychowawcaPage() {
  ...
  const handleSignOut = async () => {
    ...
    try {
      fetch("http://127.0.0.1:7245/ingest/45a9926a-fc4d-4c4f-a999-61cb01468485", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          location: "app/wychowawca/page.tsx:42",
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && !session?.user.roles?.includes("HOMEROOM")) {
  ...
  const fetchClasses = async () => {
    try {
      const res = await fetch("/api/wychowawca/classes")
      if (res.ok) {
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-gray-600">
              Wybierz klasę, aby zobaczyć oceny i wygenerować PDF
            </p>
          </div>
          <button
        ...
        {classes.length === 0 ? (
          <div className="rounded-lg bg-white p-8 text-center shadow">
            <p className="text-gray-600">Nie jesteś wychowawcą żadnej klasy.</p>
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {classes.map((class_) => (
              <Link
                key={class_.id}
                href={`/wychowawca/classes/${class_.id}`}
